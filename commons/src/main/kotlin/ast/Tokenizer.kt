...
class Tokenizer {
    ...
    private fun extractTokensFromAST(node: ASTNode): List<Token> {
        ...
        traverseAST(node, tokens)
        return tokens
    }

    private fun traverseAST(node: ASTNode, tokens: MutableList<Token>) {
        when (node) {
            is LiteralNode -> {
                val token = Token(
                    type = node.type,
            ...
            is BlockNode -> {
                node.nodes.forEach { traverseAST(it, tokens) }
            }
            is ConditionalNode -> {
                val ifToken = Token(
                ...
                if (node.elseBlock != NilNode) {
                    ...
                    traverseAST(node.elseBlock, tokens)
                }
            }
            is FunctionNode -> {
                val token = Token(
